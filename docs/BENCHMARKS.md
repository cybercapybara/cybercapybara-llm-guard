...
| Metric | How |
|---|---|
| Latency p50 / p99 / p99.9 | `make bench` (wrk) against `/healthz` (no DB) and `/health` (dependency path) |
...
./scripts/bench.sh all /health -c100 -d10s
```
...
preset on master — wrk against `/healthz` and `/health` — plus three
